...
public class FillDatabase extends DbAccess {
	...
	static void insertSongsAndTags(StringBuilder songsBuilder, StringBuilder songsTagBuilder) throws SQLException {
		...
		String insertSongTags = "insert into songtag(idsong, idtag) values " + 
				songsTagBuilder.substring(0, songsTagBuilder.length() - 1).toString() + ";";
...
//		System.out.println(insertSongTags);
	...
	public FillDatabase() throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
	...
	public void insertTracksFromAPIToDatabase(String file, int start, int end, int batchSize)
		...
		StringBuilder sql = new StringBuilder("insert into tag (idtag,name) values");
...
//		System.out.println(sql.toString());
		...
		System.out.printf("crawling finished in %f seconds", 1.0 * (e - s) / 1000.0);
		s = new Date().getTime();
...
//		System.out.println("\nOpening connection: " + (e - s));
		...
		for(int i = 0; i < tracks.size(); i++) {
			...
			songBuilder.append(",");
			songBuilder.append(hotness(track.getPlayCount() + track.getNumListeners()));
			songBuilder.append("),");
